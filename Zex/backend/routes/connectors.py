...
async def save_connector(db, user_id: str, provider: str, access_token: str,
    ...
    """Upsert connector record with encrypted tokens in auth_data."""
    expires_at = None
    if expires_in:
...
async def get_user_connectors(db, user_id: str) -> list:
    try:
        res = db.table("user_connectors").select(
            "provider_name,is_active,auth_data,updated_at"
...
async def get_connector_status(
    ...
    for r in rows:
        ...
        details.append({
            ...
            "profile": auth_data.get("profile_data") or {},
...
async def test_connector(
    ...
    try:
        res = db.table("user_connectors").select("auth_data").eq(
    ...
    auth_data = res.data.get("auth_data") or {}
    if isinstance(auth_data, str):
        try:
            auth_data = json.loads(auth_data)
        except Exception:
    ...
    enc_token = auth_data.get("access_token")
    if not enc_token:
