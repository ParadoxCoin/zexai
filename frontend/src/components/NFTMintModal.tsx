...
const NFTMintModal: React.FC<NFTMintModalProps> = ({ isOpen, onClose, image }) => {
    ...
    const [nftName, setNftName] = useState(image?.prompt?.substring(0, 30) || t('nft.defaultName'));
    const [nftDescription, setNftDescription] = useState(image?.prompt || t('nft.defaultDescription'));
    ...
    const handleMint = async () => {
        if (!account) {
            onClose(); // Close NFT modal first so wallet connect modal is visible
            connectWallet();
            return;
        }
                ...
                <div className="p-6 space-y-6">
                    ...
                    {mintStatus === 'success' ? (
                        <div className="text-center py-8 space-y-4">
                            ...
                            <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{t('nft.congratulations')}</h3>
                        ...
                        <>
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('nft.nftName')}</label>
                                    <input
                                        type="text"
                                        value={nftName}
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('nft.description')}</label>
                                    <textarea
                            ...
                            <div className="bg-purple-50 dark:bg-purple-900/10 rounded-xl p-4 border border-purple-100 dark:border-purple-800/20 flex items-center justify-between">
                                <span className="text-gray-600 dark:text-gray-400 text-sm">{t('nft.mintFee')}</span>
                                ...
                                >
                                    {isMinting ? (
                                        <>
                                            ...
                                            {mintStatus === 'approving' ? t('nft.approvingContract') : t('nft.mintingNft')}
                                        </>
                                    ) : (
                                        <>
                                            <Diamond className="w-5 h-5" />
                                            {t('nft.mintWithZex', { price: mintPrice })}
                                        </>
                                    )}
                                </button>
                            ) : (
